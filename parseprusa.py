...
def expand(name, d):
    ...
    if ctype == 'printer':
        if 'num_extruders' not in mysection:
            mysection['num_extruders'] = len(
                mysection['nozzle_diameter'].split(
...
def dump(f, name, d):
    'Dump a section to stream'
    mysection = expand(name, d)
    f.write('# generated by prusa2voron\n')
    for key in sorted([item for item in mysection
                       if item in d[f'{name.split(":")[0]}:*DEFAULTS*']]):
...
if '=' in sys.argv[2]:
    ...
    dump(sys.stdout, sys.argv[2], data)
